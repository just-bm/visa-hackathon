...
import {
    ...
    ShieldAlert, Activity, ArrowLeft, Zap, ListChecks, ChevronRight, Download
} from "lucide-react";
import { useNavigate } from "react-router";
import { exportReport } from "../api/api";
...
const Result = ({ result }) => {
    ...
    const handleDownloadReport = async () => {
        try {
            const markdown = await exportReport(result);
            const blob = new Blob([markdown], { type: "text/markdown" });
            const url = URL.createObjectURL(blob);
            const a = document.createElement("a");
            a.href = url;
            a.download = `DQS_Audit_Report_${(data.composite_dqs * 100).toFixed(0)}%.md`;
    ...
    return (
        <div className="relative min-h-screen bg-[#050505] text-white font-sans overflow-hidden selection:bg-indigo-500/30 print:bg-white print:text-black">
            {/* Background Glows */}
            ...
            <div className="relative z-10 px-6 md:px-16 lg:px-24 xl:px-32 pt-12 pb-20 print:p-0">
                <motion.div
                    ...
                    <div className="flex items-center justify-between mb-8 print:hidden">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <button
                                ...
                                className="px-5 py-2 rounded-full bg-white/5 border border-white/10 text-xs font-bold text-slate-300 hover:text-white hover:bg-white/10 transition-all flex items-center gap-2"
                            >
                            ...
                            <button onClick={() => navigate('/chat')} className="relative overflow-hidden group px-8 py-2 rounded-full bg-indigo-600 border border-indigo-500/50 text-white text-sm font-bold tracking-wide transition-all duration-300 hover:bg-indigo-500 hover:shadow-[0_0_20px_rgba(79,70,229,0.3)] active:scale-95">
